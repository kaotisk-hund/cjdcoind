...
func WriteTxWitness(w io.Writer, wit [][]byte) er.R {
...
func extractKeyOrderFromScript(script []byte, expectedPubkeys [][]byte,
	...
	if !checkIsMultiSigScript(expectedPubkeys, sigs, script) {
		return nil, ErrUnsupportedScriptType.Default()
	}
	...
	for _, p := range pubsSigs {
		...
		if pos < 0 {
			return nil, er.New("script does not contain pubkeys")
		}
...
func checkSigHashFlags(sig []byte, input *PInput) bool {
	expectedSighashType := params.SigHashAll
	if input.SighashType != 0 {
		expectedSighashType = input.SighashType
	}

	return expectedSighashType == params.SigHashType(sig[len(sig)-1])
}
...
func getKey(r io.Reader) (int, []byte, er.R) {
	...
	if err != nil {
		return -1, nil, ErrInvalidPsbtFormat.Default()
	}
	if count == 0 {
	...
	if count > MaxPsbtKeyLength {
		return -1, nil, ErrInvalidKeydata.Default()
	}
...
// exported.
func readTxOut(txout []byte) (*wire.TxOut, er.R) {
	if len(txout) < 10 {
...
func SumUtxoInputValues(packet *Packet) (int64, er.R) {
	...
	if len(packet.UnsignedTx.TxIn) != len(packet.Inputs) {
		return 0, er.Errorf("TX input length doesn't match PSBT " +
			"input length")
	}
	...
	for idx, in := range packet.Inputs {
		...
		default:
			return 0, er.Errorf("input %d has no UTXO information",
				idx)
		}
	}
	return inputSum, nil
