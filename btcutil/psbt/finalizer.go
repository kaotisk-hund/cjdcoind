...
// been finalized.
func MaybeFinalize(p *Packet, inIndex int) (bool, er.R) {
	if isFinalized(p, inIndex) {
		return true, nil
	}

	if !isFinalizable(p, inIndex) {
		return false, ErrNotFinalizable.Default()
	}
...
func Finalize(p *Packet, inIndex int) er.R {
	pInput := p.Inputs[inIndex]
...
func finalizeNonWitnessInput(p *Packet, inIndex int) er.R {
	...
	if len(sigs) < 1 || len(pubKeys) < 1 {
		return ErrNotFinalizable.Default()
	}
...
// returns an error.
func finalizeWitnessInput(p *Packet, inIndex int) er.R {
	// If this input has already been finalized, then we'll return an error
	// as we can't proceed.
	if checkFinalScriptSigWitness(p, inIndex) {
		return ErrInputAlreadyFinalized.Default()
