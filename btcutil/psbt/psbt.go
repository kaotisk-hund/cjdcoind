...
var PsbtError = er.NewErrorType("PsbtError")
...
var (
	...
	ErrInvalidPsbtFormat = PsbtError.CodeWithDetail(
		"ErrInvalidPsbtFormat", "Invalid PSBT serialization format")
	...
	ErrDuplicateKey = PsbtError.CodeWithDetail("ErrDuplicateKey", "Invalid Psbt due to duplicate key")

	// ErrInvalidKeydata indicates that a key-value pair in the PSBT
	// serialization contains data in the key which is not valid.
	ErrInvalidKeydata = PsbtError.CodeWithDetail("ErrInvalidKeydata", "Invalid key data")

	// ErrInvalidMagicBytes indicates that a passed Psbt serialization is invalid
	// due to having incorrect magic bytes.
	ErrInvalidMagicBytes = PsbtError.CodeWithDetail("ErrInvalidMagicBytes", "Invalid Psbt due to incorrect magic bytes")
	...
	ErrInvalidRawTxSigned = PsbtError.CodeWithDetail("ErrInvalidRawTxSigned", "Invalid Psbt, raw transaction must "+
		"be unsigned.")
	...
	ErrInvalidPrevOutNonWitnessTransaction = PsbtError.CodeWithDetail("ErrInvalidPrevOutNonWitnessTransaction", "Prevout hash does "+
	...
	ErrInvalidSignatureForInput = PsbtError.CodeWithDetail("ErrInvalidSignatureForInput", "Signature does not correspond "+
		"to this input")
	...
	ErrInputAlreadyFinalized = PsbtError.CodeWithDetail("ErrInputAlreadyFinalized", "Cannot finalize PSBT, finalized "+
	...
	// it is not complete
	ErrIncompletePSBT = PsbtError.CodeWithDetail("ErrIncompletePSBT", "PSBT cannot be extracted as it is "+
		"incomplete")

	// ErrNotFinalizable indicates that the PSBT struct does not have
	// sufficient data (e.g. signatures) for finalization
	ErrNotFinalizable = PsbtError.CodeWithDetail("ErrNotFinalizable", "PSBT is not finalizable")
	...
	ErrInvalidSigHashFlags = PsbtError.CodeWithDetail("ErrInvalidSigHashFlags", "Invalid Sighash Flags")
	...
	// not valid.
	ErrUnsupportedScriptType = PsbtError.CodeWithDetail("ErrUnsupportedScriptType", "Unsupported script type")
)
...
func NewFromRawBytes(r io.Reader, b64 bool) (*Packet, er.R) {
	...
	if magic != psbtMagic {
		return nil, ErrInvalidMagicBytes.Default()
	}
	...
	if GlobalType(keyint) != UnsignedTxType || keydata != nil {
		return nil, ErrInvalidPsbtFormat.Default()
	}
...
func (p *Packet) B64Encode() (string, er.R) {
	var b bytes.Buffer
