...
import (
	...
	"github.com/kaotisk-hund/cjdcoind/neutrino/banman"
...
// information about the current state of cjdcoinwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient chain.Interface) (interface{}, er.R) {
	...
	if rpc, ok := chainClient.(*chain.RPCClient); ok {
		// Call down to cjdcoind for all of the information in this command known
		// by them.
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, er.R) {
	...
	// cjdcoind returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to cjdcoind.  Therefore, create a
	...
	// texts in a non-internal cjdcoind package.
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient chain.Interface) (interface{}, er.R) {
	...
	// querying cjdcoind with getrawtransaction. We queue up a bunch of async
	...
	for _, txIn := range tx.TxIn {
		...
		if !ok {
			return nil, er.New("You must specify all transaction inputs explicitly, " +
				"or use --userpc to load them from cjdcoind.")
		}
