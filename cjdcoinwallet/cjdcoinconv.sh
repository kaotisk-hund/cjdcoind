...
usage() {
    ...
    echo "      btcd                # Change all occurrences of 'btcwallet' to 'pktwallet' and 'btcd' to 'cjdcoind'"
    echo "      rbtcd               # Change occurrences of 'pktwallet' back to 'btcwallet' and 'cjdcoind' to 'btcd'"
}
...
imports() {
    ( $FIND ./ -name '*.go' && echo './go.mod' ) | while read x; do
        echo $SED -i -e \'s@"github.com/btcsuite/btcd@"github.com/pkt-cash/cjdcoind@g\' $x;
...
rimports() {
    ( $FIND ./ -name '*.go' && echo './go.mod' ) | while read x; do
        ...
        echo $SED -i -e \'s@"github.com/pkt-cash/cjdcoind@"github.com/btcsuite/btcd@g\' $x;
...
pktwallet() {
    $FIND ./ -name '*.go' | while read x; do
        ...
        echo $SED -i -e \'s@btcd@cjdcoind@g\' $x;
    done | $RUN
}
rpktwallet() {
    $FIND ./ -name '*.go' | while read x; do
        echo $SED -i -e \'s@cjdcoind@btcd@g\' $x;
