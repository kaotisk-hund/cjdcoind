...
func (w *Wallet) FundPsbt(packet *psbt.Packet, account uint32,
	...
	addInputInfo := func(inputs []*wire.TxIn) er.R {
		...
		for idx, in := range inputs {
			...
			if err != nil {
				return er.Errorf("error fetching UTXO: %v",
					err)
			}
			...
			packet.Inputs[idx].SighashType = params.SigHashAll
	...
	case len(txIn) == 0:
		...
		tx, err = w.CreateSimpleTx(CreateTxReq{
	...
	default:
		...
		tx, err = txauthor.NewUnsignedTransaction(
			txOut, feeSatPerKB, inputSource, changeSource, false,
		)
		if err != nil {
			_ = dbtx.Rollback()
			return 0, er.Errorf("fee estimation not successful: "+
				"%v", err)
		}
		...
		err = dbtx.Commit()
		if err != nil {
			return 0, er.Errorf("could not add change address to "+
				"database: %v", err)
		}
	}
	...
	if err != nil {
		return 0, er.Errorf("could not sort PSBT: %v", err)
	}
...
func (w *Wallet) FinalizePsbt(packet *psbt.Packet) er.R {
	...
	for idx, txIn := range tx.TxIn {
		...
		if err != nil {
			return er.Errorf("error computing input script for "+
				"input %d: %v", idx, err)
		}
		...
		if err != nil {
			return er.Errorf("error serializing witness: %v", err)
		}
...
func constantInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	return func(target btcutil.Amount) (
		btcutil.Amount,
		[]*wire.TxIn,
		[]wire.TxInAdditional,
		er.R,
