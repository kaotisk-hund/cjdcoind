...
import (
	...
	"github.com/kaotisk-hund/cjdcoind/cjdcoinwallet/internal/cfgutil"
	"github.com/kaotisk-hund/cjdcoind/cjdcoinwallet/internal/legacy/keystore"
	"github.com/kaotisk-hund/cjdcoind/cjdcoinwallet/netparams"
	"github.com/kaotisk-hund/cjdcoind/cjdcoinwallet/wallet"
)

const (
	defaultCAFilename       = "cjdcoind.cert"
	defaultConfigFilename   = "cjdcoinwallet.conf"
...
var (
	cjdcoindDefaultCAFile  = filepath.Join(btcutil.AppDataDir("cjdcoind", false), "rpc.cert")
	cjdcoindDefaultConf    = filepath.Join(btcutil.AppDataDir("cjdcoind", false), "cjdcoind.conf")
	defaultAppDataDir  = btcutil.AppDataDir("cjdcoinwallet", false)
...
type config struct {
	...
	Wallet        string                  `short:"w" long:"wallet" description:"Wallet file name or path, if a simple word such as 'personal' then cjdcoinwallet will look for wallet_personal.db, if prefixed with a / then cjdcoinwallet will consider it an absolute path."`
	TestNet3      bool                    `long:"testnet" description:"Use the test Bitcoin network (version 3) (default mainnet)"`
	PktTestNet    bool                    `long:"cjdcointest" description:"Use the test cjdcoin.cash test network"`
	BtcMainNet    bool                    `long:"btc" description:"Use the test bitcoin main network"`
	PktMainNet    bool                    `long:"cjdcoin" description:"Use the test cjdcoin.cash main network"`
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of cjdcoind RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with cjdcoind"`
	DisableClientTLS bool                    `long:"noclienttls" description:"nolonger used" hidden:"true"`
	ClientTLS        bool                    `long:"clienttls" description:"enable tls to the cjdcoind instance"`
	BtcdUsername     string                  `long:"cjdcoindusername" description:"Username for cjdcoind authentication"`
	BtcdPassword     string                  `long:"cjdcoindpassword" default-mask:"-" description:"Password for cjdcoind authentication"`
	...
	UseRPC                 bool                    `long:"userpc" description:"Use an RPC connection to cjdcoind rather than using neutrino, the default behavior is to connect to a single local cjdcoind instance using neutrino, UseSPV will make neutrino connect to multiple nodes"`
	...
	Username               string                  `short:"u" long:"rpcuser" description:"Username for legacy RPC and cjdcoind authentication (if cjdcoindusername is unset)"`
	Password               string                  `short:"P" long:"rpcpass" default-mask:"-" description:"Password for legacy RPC and cjdcoind authentication (if cjdcoindpassword is unset)"`
...
//
// The above results in cjdcoinwallet functioning properly without any config
...
func loadConfig() (*config, []string, er.R) {
	...
	// Attempt to grab the user/pass from cjdcoind.conf
	...
	} else if userpass, err := cjdcoinconfig.ReadUserPass(cjdcoindDefaultConf); err != nil {
	...
	if errr := flags.NewIniParser(parser).ParseFile(configFilePath); errr != nil {
		...
		if configFileError = cjdcoinconfig.CreateDefaultConfigFile(
			configFilePath, cjdcoinconfig.PktwalletSampleConfig); configFileError != nil {
		} else {
	...
	if !cfg.UseRPC {
		...
		if cfg.ClientTLS {
			// If CAFile is unset, choose either the copy or local cjdcoind cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local cjdcoind and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						cjdcoindCertExists, err := cfgutil.FileExists(
							cjdcoindDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						}
						if cjdcoindCertExists {
							cfg.CAFile.Value = cjdcoindDefaultCAFile
						}
					}
				}
			}
	...
	// If the cjdcoind username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for cjdcoind and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for cjdcoind and wallet.
	if cfg.BtcdUsername == "" {
