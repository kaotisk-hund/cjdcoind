...
import (
	...
	"github.com/kaotisk-hund/cjdcoind/limits"
...
// winServiceMain is only invoked on Windows.  It detects when cjdcoind is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, er.R)

// cjdcoindMain is the real main function for cjdcoind.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func cjdcoindMain(serverChan chan<- *server) er.R {
	...
	// Perform upgrades to cjdcoind as new versions require it.
...
func main() {
	version.SetUserAgentName("cjdcoind")
	...
	if err := cjdcoindMain(nil); err != nil {
		os.Exit(1)
	}
}
