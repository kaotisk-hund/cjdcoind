...
type DB interface {
	...
	CreateTower(*lnwire.NetAddress) (*wtdb.Tower, er.R)
	...
	LoadTower(*btcec.PublicKey) (*wtdb.Tower, er.R)

	// LoadTowerByID retrieves a tower by its tower ID.
	LoadTowerByID(wtdb.TowerID) (*wtdb.Tower, er.R)

	// ListTowers retrieves the list of towers available within the
	// database.
	ListTowers() ([]*wtdb.Tower, er.R)
	...
	// same index.
	NextSessionKeyIndex(wtdb.TowerID) (uint32, er.R)
	...
	// restarts.
	CreateClientSession(*wtdb.ClientSession) er.R
	...
	ListClientSessions(*wtdb.TowerID) (map[wtdb.SessionID]*wtdb.ClientSession, er.R)

	// FetchChanSummaries loads a mapping from all registered channels to
	// their channel summaries.
	FetchChanSummaries() (wtdb.ChannelSummaries, er.R)
	...
	RegisterChannel(lnwire.ChannelID, []byte) er.R
	...
	// different policy.
	MarkBackupIneligible(chanID lnwire.ChannelID, commitHeight uint64) er.R
	...
	AckUpdate(id *wtdb.SessionID, seqNum, lastApplied uint16) er.R
}
...
type ECDHKeyRing interface {
	...
	// key.
	DeriveKey(keyLoc keychain.KeyLocator) (keychain.KeyDescriptor, er.R)
}
