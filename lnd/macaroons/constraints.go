...
type Constraint func(*macaroon.Macaroon) er.R
...
func AddConstraints(mac *macaroon.Macaroon, cs ...Constraint) (*macaroon.Macaroon, er.R) {
	newMac := mac.Clone()
...
func TimeoutConstraint(seconds int64) func(*macaroon.Macaroon) er.R {
	return func(mac *macaroon.Macaroon) er.R {
...
func IPLockConstraint(ipAddr string) func(*macaroon.Macaroon) er.R {
	return func(mac *macaroon.Macaroon) er.R {
		if ipAddr != "" {
			macaroonIPAddr := net.ParseIP(ipAddr)
			if macaroonIPAddr == nil {
				return er.Errorf("incorrect macaroon IP-lock address")
			}
