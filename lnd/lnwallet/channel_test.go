...
func testAddSettleWorkflow(t *testing.T, tweakless bool) {
	...
	mSatTransferred := lnwire.NewMSatFromSatoshis(btcutil.UnitsPerCoin())
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := btcutil.Amount(btcutil.UnitsPerCoin()*4) -
	...
	expectedBalance := btcutil.Amount(btcutil.UnitsPerCoin() * 5)
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	aliceBal := lnwire.NewMSatFromSatoshis(btcutil.UnitsPerCoin())
...
// in memory state.
func restartChannel(channelOld *LightningChannel) (*LightningChannel, er.R) {
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMSatFromSatoshis(5 * btcutil.UnitsPerCoin())
	bobBalance := lnwire.NewMSatFromSatoshis(5 * btcutil.UnitsPerCoin())
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshisF(4.1 * btcutil.UnitsPerCoinF())
	htlc, _ := createHTLC(0, htlcAmt)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMSatFromSatoshis(btcutil.UnitsPerCoin() * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshisF(1.5 * btcutil.UnitsPerCoinF())
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel, func()) {
		...
		bobMinReserve := btcutil.Amount(6 * btcutil.UnitsPerCoin())
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshisF(0.5 * btcutil.UnitsPerCoinF())
	...
	assertChannelBalances(
		...
		btcutil.Amount(btcutil.UnitsPerCoinF()*4.5)-commitFee, btcutil.UnitsPerCoin()*5,
	)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshisF(3.5 * btcutil.UnitsPerCoinF())
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		btcutil.UnitsPerCoin()*3-commitFee, btcutil.UnitsPerCoin()*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		btcutil.UnitsPerCoin()*3-commitFee, btcutil.UnitsPerCoin()*7,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		btcutil.UnitsPerCoin()*3-commitFee, btcutil.UnitsPerCoin()*6,
	)
}
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*btcutil.UnitsPerCoin() - commitFee
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*btcutil.UnitsPerCoin() - commitFee - htlcSat
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMSatFromSatoshisF(0.1 * btcutil.UnitsPerCoinF())
...
func compareHtlcs(htlc1, htlc2 *PaymentDescriptor) er.R {
	if htlc1.LogIndex != htlc2.LogIndex {
		return er.Errorf("htlc log index did not match")
	}
	if htlc1.HtlcIndex != htlc2.HtlcIndex {
		return er.Errorf("htlc index did not match")
	}
	if htlc1.ParentIndex != htlc2.ParentIndex {
		return er.Errorf("htlc parent index did not match")
	}

	if htlc1.RHash != htlc2.RHash {
		return er.Errorf("htlc rhash did not match")
	}
	return nil
}

// compareIndexes is a helper method to compare two index maps.
func compareIndexes(a, b map[uint64]*list.Element) er.R {
	for k1, e1 := range a {
		e2, ok := b[k1]
		if !ok {
			return er.Errorf("element with key %d "+
				"not found in b", k1)
		}
	...
	for k1, e1 := range b {
		e2, ok := a[k1]
		if !ok {
			return er.Errorf("element with key %d not "+
				"found in a", k1)
		}
		htlc1, htlc2 := e1.Value.(*PaymentDescriptor), e2.Value.(*PaymentDescriptor)
		if err := compareHtlcs(htlc1, htlc2); err != nil {
...
func compareLogs(a, b *updateLog) er.R {
	if a.logIndex != b.logIndex {
		return er.Errorf("log indexes don't match: %d vs %d",
			a.logIndex, b.logIndex)
	}

	if a.htlcCounter != b.htlcCounter {
		return er.Errorf("htlc counters don't match: %d vs %d",
			a.htlcCounter, b.htlcCounter)
	}

	if err := compareIndexes(a.updateIndex, b.updateIndex); err != nil {
		return er.Errorf("update indexes don't match: %v", err)
	}
	if err := compareIndexes(a.htlcIndex, b.htlcIndex); err != nil {
		return er.Errorf("htlc indexes don't match: %v", err)
	}

	if a.Len() != b.Len() {
		return er.Errorf("list lengths not equal: %d vs %d",
			a.Len(), b.Len())
	}
