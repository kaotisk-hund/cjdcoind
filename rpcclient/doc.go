...
Bitcoin RPC server that uses a cjdcoind/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with cjdcoind (https://github.com/kaotisk-hund/cjdcoind),
cjdcoinwallet (https://github.com/cjdcoin-cash/cjdcoinwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, cjdcoind and
cjdcoinwallet provide a websocket interface that is more efficient than the standard
...
cjdcoind or cjdcoinwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by cjdcoind and
cjdcoinwallet only uses a single connection that remains open and allows
...
All notifications provided by cjdcoind require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by cjdcoind (and
cjdcoinwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that cjdcoind intentionally separates the wallet
functionality into a separate process named cjdcoinwallet.  This means if you are
connected to the cjdcoind RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, cjdcoinwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - cjdcoindwebsockets
   Connects to a cjdcoind RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - cjdcoinwalletwebsockets
   Connects to a cjdcoinwallet RPC server using TLS-secured websockets, registers
