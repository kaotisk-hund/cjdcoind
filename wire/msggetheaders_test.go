...
func TestGetHeadersWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{baseGetHeaders, baseGetHeadersEncoded, pver, BaseEncoding, 0, er.E(io.ErrShortWrite), er.E(io.EOF)},
		// Force error in block locator hash count.
		{baseGetHeaders, baseGetHeadersEncoded, pver, BaseEncoding, 4, er.E(io.ErrShortWrite), er.E(io.EOF)},
		// Force error in block locator hashes.
		{baseGetHeaders, baseGetHeadersEncoded, pver, BaseEncoding, 5, er.E(io.ErrShortWrite), er.E(io.EOF)},
		// Force error in stop hash.
		{baseGetHeaders, baseGetHeadersEncoded, pver, BaseEncoding, 69, er.E(io.ErrShortWrite), er.E(io.EOF)},
